...
function single_post(){
	if (isset($_GET['post_id'])){
		...
		<button class='btn btn-info pull-right' name='reply'>Comments</button>
		</form>
		</div>
		</div>
		</div>
		...
		if(isset($_POST['reply'])){
			...
			if($comment == ""){
				...
				echo"<script>alert('Enter your comment!')</scrpt>";
				echo "<script>window.open('single.php?post_id=post_id','_self')</script>";
			}
		}
	...
	while($row_posts=mysqli_fetch_array($run_posts)){
		...
		$user = "selsect * from users where user_id='$user_id' AND posts='yes'";
